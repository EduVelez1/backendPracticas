...
const index = async (req, res, next) => {
  try {
    const docs = await Curricular.findAll({
      include: [
        Student,
        Institution,
        Course,
        ...
        ObjectiveCurricular,
        CriteriaCurricular,
        MetodologiaCurricular,
        ResourceCurricular,
        ResultFinalCurricular,
      ],
    });
    res.json({
      docs,
...
const findById = async (req, res, next) => {
  ...
  try {
    const docs = await Curricular.findOne({
      where: { id },
      include: [
        Student,
        Institution,
...
const create = async (req, res, next) => {
  ...
  try {
    ...
    const courses = await Course.findAll({
      where: { id: { [Op.in]: coursesArray } },
    });
    const teachers = await Teacher.findAll({
      where: { id: { [Op.in]: teachersArray } },
    });
